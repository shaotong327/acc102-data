...
try:
    ...
    st.error(
        "⚠️ Data files `prices_wide.csv` and/or `returns_wide.csv` not found. "
        "Please run the notebook `stock_risk_analysis_fixed.ipynb` first to "
        "generate these files from WRDS CRSP."
    )
    st.stop()
