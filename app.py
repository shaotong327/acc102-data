...
# ═══════════════════════════════════════════════════════════════════════════════
# IMPORT GUARD — give a friendly error message if plotly is missing
# (this prevents the cryptic "ModuleNotFoundError" page on Streamlit Cloud)
# ═══════════════════════════════════════════════════════════════════════════════
try:
    import plotly.express as px
    import plotly.graph_objects as go
except ModuleNotFoundError:
    st.set_page_config(page_title="Stock Risk Analyser", page_icon="📈")
    st.error(
        "❌ **Missing dependency: `plotly`**\n\n"
        "This app requires `plotly` to render charts. It looks like it was not "
        "installed in the deployment environment.\n\n"
        "**If you are deploying to Streamlit Community Cloud:**\n"
        "1. Make sure your repo contains a file named exactly `requirements.txt` "
        "(not `requirements_streamlit_cloud.txt` or similar)\n"
        "2. The file should include this line:\n"
        "```\nplotly==5.24.1\n```\n"
        "3. Push the change to GitHub and click **Reboot app** in Streamlit Cloud.\n\n"
        "**If you are running locally:** run `pip install plotly` in your terminal."
    )
    st.stop()
...
@st.cache_data
def load_data():
    prices  = pd.read_csv('prices_wide.csv',  index_col='date', parse_dates=['date'])
    returns = pd.read_csv('returns_wide.csv', index_col='date', parse_dates=['date'])
    return prices, returns


try:
    prices_full, returns_full = load_data()
except FileNotFoundError:
    st.error(
        "⚠️ Data files `prices_wide.csv` and/or `returns_wide.csv` not found in the "
        "repository root.\n\n"
        "**To fix:**\n"
        "1. Run the notebook `stock_risk_analysis_fixed.ipynb` to generate these "
        "files from WRDS CRSP\n"
        "2. Push them to GitHub alongside `app.py`\n"
